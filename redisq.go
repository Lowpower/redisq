...
func (s TaskState) String() string {
	...
	panic("redisq: unknown task state")
}
...
func ParseRedisURI(uri string) (RedisConnOpt, error) {
	u, err := url.Parse(uri)
	if err != nil {
		return nil, fmt.Errorf("redisq: could not parse redis uri: %v", err)
	}
	switch u.Scheme {
	case "redis", "rediss":
	...
	default:
		return nil, fmt.Errorf("redisq: unsupported uri scheme: %q", u.Scheme)
	}
}
...
func parseRedisURI(u *url.URL) (RedisConnOpt, error) {
	...
	if len(u.Path) > 0 {
		...
		if err != nil {
			return nil, fmt.Errorf("redisq: could not parse redis uri: database number should be the first segment of the path")
		}
	}
	var password string
...
func parseRedisSocketURI(u *url.URL) (RedisConnOpt, error) {
	const errPrefix = "redisq: could not parse redis socket uri"
	if len(u.Path) == 0 {
